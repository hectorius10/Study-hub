...
function creerCaseJour({ date, horsMois }) {
    ...
    if (sessionsCachees > 0) {
        ...
        // NOUVEAUTÉ : Affiche le détail des sessions cachées au survol
        const texteCaches = sessions
            .slice(MAX_ETIQUETTES_VISIBLES)
            .map(s => `${s.label} – ${formatDuree(s.duree)}`)
            .join("\n");
        compteur.title = texteCaches;
...
function creerEtiquetteSession(session) {
    ...
    const texte = `${session.label} – ${formatDuree(session.duree)}`;
    etiquette.textContent = texte;
    
    // NOUVEAUTÉ : Ajout de l'infobulle native pour l'affichage au survol
    etiquette.title = texte; 
